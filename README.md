...
本仓库 PR 合并
  ...
  -> Builtin: unlisted + auto-on
     Marketplace: listed + tenant subscription
```
...
`auto_update=review`。Builtin 批准后进入数据库运行集但保持 unlisted；
Marketplace 批准后才进入可发现、可订阅状态。不存在 filesystem
