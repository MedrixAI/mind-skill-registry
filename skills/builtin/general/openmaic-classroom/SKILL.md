...
license: MIT
metadata:
  mind.id: ai.medrix.skill.builtin.openmaic-classroom
...
当场景 2 有 RAG 检索结果时，先读取 `scripts/rag-to-requirement.py`，再通过任务临时文件用通用 shell 将 chunks 转换为 requirement。Skill 文件不会自动出现在 shell workspace。

1. 调用 `read_skill(skill_ref="<当前 Skill 的 canonical skill_ref>", file_path="scripts/rag-to-requirement.py")` 取得脚本文本。
2. 将脚本文本作为 stdin 安全写入任务临时文件，并读取 stdout 返回的路径：

```text
shell(
  command="tmp=$(mktemp /tmp/openmaic-rag.XXXXXX.py) && cat > \"$tmp\" && printf '%s\\n' \"$tmp\"",
  stdin=<read_skill 返回的脚本文本>
)
```

3. 执行返回的临时文件，把 chunks JSON 作为业务 stdin，并在退出时清理文件：

```text
shell(
  command="tmp='<上一步返回路径>'; trap 'rm -f \"$tmp\"' EXIT; python \"$tmp\"",
  stdin="{\"chunks\": [...检索结果...], \"query\": \"用户查询\", \"audience\": \"目标受众\"}"
)
```

**stdin 数据格式（JSON 字符串，不可用 `--file` 代替）：**
...
**注意：**
- 必须将 chunks 数据作为执行调用的 `stdin` 传入，不要拼接进 `command`
...
对每个 concept，读取 `scripts/concept-to-requirement.py` 后，通过任务临时文件将 concept + 关联 entity 转换为 requirement：

1. 调用 `read_skill(skill_ref="<当前 Skill 的 canonical skill_ref>", file_path="scripts/concept-to-requirement.py")` 取得脚本文本。
2. 将脚本文本作为 stdin 写入任务临时文件：

```text
shell(
  command="tmp=$(mktemp /tmp/openmaic-concept.XXXXXX.py) && cat > \"$tmp\" && printf '%s\\n' \"$tmp\"",
  stdin=<read_skill 返回的脚本文本>
)
```

3. 执行 stdout 返回的路径，把 concept JSON 放在业务 stdin，并在退出时清理：

```text
shell(
  command="tmp='<上一步返回路径>'; trap 'rm -f \"$tmp\"' EXIT; python \"$tmp\"",
  stdin="{\"concept\": {\"slug\": \"...\", \"title\": \"...\", \"summary\": \"...\", \"content\": \"...\"}, \"entities\": [{\"slug\": \"...\", \"title\": \"...\", \"summary\": \"...\", \"link_type\": \"...\"}], \"language\": \"zh-CN\", \"depth\": \"intermediate\"}"
)
```
...
- 脚本只在当前 Agent 已授权通用 shell 时执行；工作目录、超时与隔离策略沿用该通用执行面
